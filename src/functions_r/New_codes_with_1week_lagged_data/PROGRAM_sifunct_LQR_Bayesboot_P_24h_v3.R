...
source(paste0(str1,"Function_LQR_Bayesboot_P_24h_v3.R"), echo = F)

load(paste0(str1,"DATA_tra_v3.RData"))
load(paste0(str1,"DATA_for_v3.RData"))
...
pred_tra[,c(1,3,5)] <- (pred_tra[,c(1,3,5)]-base_P$xmin)/(base_P$xmax-base_P$xmin)
pred_for[,c(1,3,5)] <- (pred_for[,c(1,3,5)]-base_P$xmin)/(base_P$xmax-base_P$xmin)

base_Q <- list("xmin" = min(Q_tra), "xmax" = max(Q_tra))
Q_tra <- (Q_tra-base_Q$xmin)/(base_Q$xmax-base_Q$xmin)
pred_tra[,c(2,4,6)] <- (pred_tra[,c(2,4,6)]-base_Q$xmin)/(base_Q$xmax-base_Q$xmin)
pred_for[,c(2,4,6)] <- (pred_for[,c(2,4,6)]-base_Q$xmin)/(base_Q$xmax-base_Q$xmin)

Results_P <- array(rep(NaN, 144*3), c(144,3))    # In real-time usage, this line is deleted.
...
{
  print(paste0(h, "/ 144"))
  Results_P[h,c(1,2,3)] <- LQR_Bayesboot(P_tra, pred_tra, pred_for[h,], h, N_boot)
}
...
save("Results_P","base_P","base_Q","Results_abs_P","P_for","Q_for", file = paste0(str1,"Results_pask_P_v3.RData"))
write.csv("Results_abs_P", paste0(str1,"Results_pask_P_v3.csv")) 
