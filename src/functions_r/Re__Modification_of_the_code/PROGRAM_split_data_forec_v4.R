...
Plag145_for <- array(rep(0,144))
Qlag145_for <- array(rep(0,144))
for (i in (L-144+1):(L)) 
{ Plag145_for[i-(L-144)] <- P[i-145]
  Qlag145_for[i-(L-144)] <- Q[i-145] }

Plag289_for <- array(rep(0,144))
Qlag289_for <- array(rep(0,144))
for (i in (L-144+1):(L)) 
{ Plag289_for[i-(L-144)] <- P[i-289]
  Qlag289_for[i-(L-144)] <- Q[i-289] }
...
Plag2_for <- array(rep(0,144))
Qlag2_for <- array(rep(0,144))
for (i in (L-144+1):(L))
{ int <- i %% 144
  if (int >= 1 && int <= 108)
  { Plag_for[i-(L-144)] <- Plag144_for[i-(L-144)]
    Qlag_for[i-(L-144)] <- Qlag144_for[i-(L-144)]
    Plag2_for[i-(L-144)] <- Plag145_for[i-(L-144)]
    Qlag2_for[i-(L-144)] <- Qlag145_for[i-(L-144)] }
else
  { Plag_for[i-(L-144)] <- Plag288_for[i-(L-144)]
    Qlag_for[i-(L-144)] <- Qlag288_for[i-(L-144)]
    Plag2_for[i-(L-144)] <- Plag289_for[i-(L-144)]
    Qlag2_for[i-(L-144)] <- Qlag289_for[i-(L-144)] }
}
...
# Pool the predictors
pred_for <- data.frame(Plag_for, Qlag_for, Plag2_for, Qlag2_for, Plag1008_for, Qlag1008_for, avgP_for, avgQ_for)

## SAVE DATA TRAINING -----------------------------------------------------------------------------
save(pred_for, P_for, Q_for, file = paste0(str1,"Test_2021_09_07 load_24h/DATA_for_v4.RData"))
