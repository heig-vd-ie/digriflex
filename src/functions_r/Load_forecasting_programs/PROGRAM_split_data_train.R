...
Plag144_tra <- array(rep(0,L-288-144))
Qlag144_tra <- array(rep(0,L-288-144))
for (i in (288+1):(L-144)) 
{ Plag144_tra[i-288] <- P[i-144]
  Qlag144_tra[i-288] <- Q[i-144] }

Plag288_tra <- array(rep(0,L-288-144))
Qlag288_tra <- array(rep(0,L-288-144))
for (i in (288+1):(L-144)) 
{ Plag288_tra[i-288] <- P[i-288]
  Qlag288_tra[i-288] <- Q[i-288] }

# Selecting one or two days before, depending on the hour of the day --------------
Plag_tra <- array(rep(0,L-288-144))
Qlag_tra <- array(rep(0,L-288-144))
for (i in (288+1):(L-144))
{ int <- i %% 144
  if (int >= 1 && int <= 108)
  { Plag_tra[i-288] <- Plag144_tra[i-288]
    Qlag_tra[i-288] <- Qlag144_tra[i-288] }
  else
  { Plag_tra[i-288] <- Plag288_tra[i-288]
    Qlag_tra[i-288] <- Qlag288_tra[i-288] }
}

# Average weather condition during the last observable day ------------------------
avgP_tra <- array(rep(0,L-288-144))
avgQ_tra <- array(rep(0,L-288-144))
for (i in (288+1):(L-144)) 
{ D <- ((i-1) %/% 144)+1
  avgP_tra[i-288] <- mean(P[(144*(D-2)+1):(144*(D-2)+108)])
  avgQ_tra[i-288] <- mean(Q[(144*(D-2)+1):(144*(D-2)+108)]) }

pred_tra <- data.frame(Plag_tra, Qlag_tra, avgP_tra, avgQ_tra)

## SAVE DATA TRAINING -----------------------------------------------------------------------------
save(pred_tra, P_tra, Q_tra, file = paste0(str1,"Test_2021_09_07 load_24h/DATA_tra.RData"))
