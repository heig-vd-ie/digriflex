...
source(paste0(str1,"Function_LQR_Bayesboot_P_24h_v2.R"), echo = F)
...
pred_tra[,c(1,3)] <- (pred_tra[,c(1,3)]-base_P$xmin)/(base_P$xmax-base_P$xmin)
pred_for[,c(1,3)] <- (pred_for[,c(1,3)]-base_P$xmin)/(base_P$xmax-base_P$xmin)

base_Q <- list("xmin" = min(Q_tra), "xmax" = max(Q_tra))
Q_tra <- (Q_tra-base_Q$xmin)/(base_Q$xmax-base_Q$xmin)
pred_tra[,c(2,4)] <- (pred_tra[,c(2,4)]-base_Q$xmin)/(base_Q$xmax-base_Q$xmin)
pred_for[,c(2,4)] <- (pred_for[,c(2,4)]-base_Q$xmin)/(base_Q$xmax-base_Q$xmin)
...
save("Results_P","base_P","base_Q","Results_abs_P","P_for","Q_for", file = paste0(str1,"Results_pask_P_v2.RData"))
write.csv("Results_abs_P", paste0(str1,"Results_pask_P_v2.csv")) 
