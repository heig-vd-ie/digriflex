LQR_Bayesboot <- function(irra_tra,pred_tra,pred_for,h,N_boot) {
  ...
  irra_prev[c(1,5,9),] <- mean(irra_prev[c(1,5,9),]) # This line will change after the optimization of the sample pick
  irra_prev <- base_irra$xmin + irra_prev[c(1,5,9),]*(base_irra$xmax-base_irra$xmin)
}
