...
source(paste0(str1,"Function_LQR_Bayesboot_irra_24h_v2.R"), echo = F)

load(paste0(str1,"DATA_tra.RData")) 
N_boot <- 100       # This is the desired number of bootstrap samples.

Results <- array(rep(NaN, 144*3), c(144,3))    # In real-time usage, this line is deleted.
...
{ print(h)
  Results[h,c(1,2,3)] <- LQR_Bayesboot(irra_tra,pred_tra,pred_for[h,],h,N_boot) }
