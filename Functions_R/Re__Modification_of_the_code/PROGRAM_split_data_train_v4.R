...
Plag145_tra <- array(rep(0,L-1008-144))
Qlag145_tra <- array(rep(0,L-1008-144))
for (i in (1008+1):(L-144)) 
{ Plag145_tra[i-1008] <- P[i-145]
  Qlag145_tra[i-1008] <- Q[i-145] }

Plag289_tra <- array(rep(0,L-1008-144))
Qlag289_tra <- array(rep(0,L-1008-144))
for (i in (1008+1):(L-144)) 
{ Plag289_tra[i-1008] <- P[i-289]
  Qlag289_tra[i-1008] <- Q[i-289] }
...
Plag2_tra <- array(rep(0,L-1008-144))
Qlag2_tra <- array(rep(0,L-1008-144))
...
{ int <- i %% 144
  ...
  { Plag_tra[i-1008] <- Plag144_tra[i-1008]
    ...
    Plag2_tra[i-1008] <- Plag145_tra[i-1008]
    Qlag2_tra[i-1008] <- Qlag145_tra[i-1008] }
  else
  { Plag_tra[i-1008] <- Plag288_tra[i-1008]
    Qlag_tra[i-1008] <- Qlag288_tra[i-1008]
    Plag2_tra[i-1008] <- Plag289_tra[i-1008]
    Qlag2_tra[i-1008] <- Qlag289_tra[i-1008]}
}
...
pred_tra <- data.frame(Plag_tra, Qlag_tra, Plag2_tra, Qlag2_tra, Plag1008_tra, Qlag1008_tra, avgP_tra, avgQ_tra)

## SAVE DATA TRAINING -----------------------------------------------------------------------------
save(pred_tra, P_tra, Q_tra, file = paste0(str1,"Test_2021_09_07 load_24h/DATA_tra_v4.RData"))
