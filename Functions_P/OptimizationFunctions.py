...
#### To Do lists:
## - CM#0: - reamined reviewing from functions da_optimization_robust
...
def rt_following_digriflex(grid_inp, P_net, Q_net, pv_forecast, SOC_battery):
    ...
    Battery_P_cap_p = min([grid_inp["storage_elements"][0]["P_max_kW"],
                          (grid_inp["storage_elements"][0]["SOC_max_kWh"]-SOC_battery) * 6])
    Battery_P_cap_n = max([-grid_inp["storage_elements"][0]["P_max_kW"],
                           (grid_inp["storage_elements"][0]["SOC_min_kWh"]-SOC_battery) * 6])
    Battery_Q_cap = np.sqrt(grid_inp["storage_elements"][0]["S_max_kVA"] ** 2 -
                            max([Battery_P_cap_p, - Battery_P_cap_n]) ** 2)
    ABB_P_sp = min([ABB_P_cap, P_net, pv_forecast])
    Battery_P_sp = max([min([P_net - ABB_P_sp, Battery_P_cap_p]), Battery_P_cap_n])
    Battery_Q_sp = max([min([Q_net, Battery_Q_cap]), -Battery_Q_cap])
    return ABB_P_sp, Battery_P_sp, Battery_Q_sp
...
def DA_Optimization_Robust(case_name, case_inp, grid_inp, meas_inp, fore_inp, output_DF):
    ...
    for n in grid_inp["lines"]:
        ...
        Line_Smax[af.find_n(n["bus_j"], grid_inp["buses"]),
                  af.find_n(n["bus_k"], grid_inp["buses"])] = n["Cap"]
        Line_Vbase[af.find_n(n["bus_j"], grid_inp["buses"]),
                   af.find_n(n["bus_k"], grid_inp["buses"])] = grid_inp["buses"][af.find_n(n["bus_k"], grid_inp["buses"])][
            "U_kV"]
        Line_Zre[af.find_n(n["bus_j"], grid_inp["buses"]),
    ...
    for t in Time_Set:
        ...
        PROB_DA.addConstr(sigma_p_P[t] * sigma_p_P[t] >= sum(sigma_p_ConPoint_fac_P[f, t] * ConPoint_P_DA_RS_pos[f, t]
                                                             * sigma_p_ConPoint_fac_P[f, t] * ConPoint_P_DA_RS_pos[f, t] for f in ConPoint_Set)
        ...
        PROB_DA.addConstr(sigma_n_P[t] * sigma_n_P[t] >= sum(sigma_n_ConPoint_fac_P[f, t] * ConPoint_P_DA_RS_neg[f, t]
                                                             * sigma_n_ConPoint_fac_P[f, t] * ConPoint_P_DA_RS_neg[f, t] for f in ConPoint_Set)
        ...
        PROB_DA.addConstr(sigma_p_Q[t] * sigma_p_Q[t] >= sum(sigma_p_ConPoint_fac_Q[f, t] * ConPoint_Q_DA_RS_pos[f, t]
                                                             * sigma_p_ConPoint_fac_Q[f, t] * ConPoint_Q_DA_RS_pos[f, t] for f in ConPoint_Set)
        ...
        PROB_DA.addConstr(sigma_n_Q[t] * sigma_n_Q[t] >= sum(sigma_n_ConPoint_fac_Q[f, t] * ConPoint_Q_DA_RS_neg[f, t]
                                                             * sigma_n_ConPoint_fac_Q[f, t] * ConPoint_Q_DA_RS_neg[f, t] for f in ConPoint_Set)
                          + sum(sigma_p_DM_Q[d, t] * sigma_p_DM_Q[d, t] for d in DM_Set))
        PROB_DA.addConstr(zeta_p_P[t] == sum(zeta_p_ConPoint_fac_P[f, t] * ConPoint_P_DA_RS_pos[f, t] for f in ConPoint_Set)
                          + sum(zeta_n_DM_P[d, t] for d in DM_Set) + sum(zeta_n_PV[i, t] for i in PV_Set))
        PROB_DA.addConstr(zeta_n_P[t] == sum(zeta_n_ConPoint_fac_P[f, t] * ConPoint_P_DA_RS_neg[f, t] for f in ConPoint_Set)
                          + sum(zeta_p_DM_P[d, t] for d in DM_Set) + sum(zeta_p_PV[i, t] for i in PV_Set))
        PROB_DA.addConstr(zeta_p_Q[t] == sum(zeta_p_ConPoint_fac_Q[f, t] * ConPoint_Q_DA_RS_pos[f, t] for f in ConPoint_Set)
                          + sum(zeta_n_DM_Q[d, t] for d in DM_Set))
        PROB_DA.addConstr(zeta_n_Q[t] == sum(zeta_n_ConPoint_fac_Q[f, t] * ConPoint_Q_DA_RS_neg[f, t] for f in ConPoint_Set)
        ...
        for i in PV_Set:
            ...
            PROB_DA.addConstr(max_PV_P[i, t] * max_PV_P[i, t]
                              ...
                              * (min_PV_Q[i, t] + PV_V_grid[i] * PV_V_grid[i] * PV_cap[i] / PV_X[i])
                              <= (PV_cap[i] * PV_V_grid[i] * PV_V_conv[i] / PV_X[i])
                              * (PV_cap[i] * PV_V_grid[i] * PV_V_conv[i] / PV_X[i]))
            PROB_DA.addConstr(max_PV_P[i, t] * max_PV_P[i, t] + max_PV_Q[i, t] * max_PV_Q[i, t] <= PV_cap[i] * PV_cap[i])
            PROB_DA.addConstr(max_PV_P[i, t] * max_PV_P[i, t] + min_PV_Q[i, t] * min_PV_Q[i, t] <= PV_cap[i] * PV_cap[i])
        ...
        for f in ConPoint_Set:
            ...
            PROB_DA.addConstr(sum((max_Vmag_sq[n, t] - Vmag_sq[n, t]) * ConPoint_Inc_Mat[f, n] for n in Node_Set)
                              == 2 * Vmag_sq[n, t] * zeta_p_ConPoint_Vmag[f, t])
            PROB_DA.addConstr(sum((Vmag_sq[n, t] - min_Vmag_sq[n, t]) * ConPoint_Inc_Mat[f, n] for n in Node_Set)
                              == 2 * zeta_n_ConPoint_Vmag[f, t] * Vmag_sq[n, t])        
        # Adjustable robust constraints (nodes and branches)
        for n in Node_Set:
            PROB_DA.addConstr(zeta_p_Net_P[n, t] == sum((zeta_p_PV[i, t] + alfa_PV_P_p[i, t]) * PV_Inc_Mat[i, n] for i in PV_Set)
            ...
            PROB_DA.addConstr(zeta_n_Net_P[n, t] == sum((zeta_n_PV[i, t] + alfa_PV_P_n[i, t]) * PV_Inc_Mat[i, n] for i in PV_Set)
...
def DA_Optimization(case_name, case_inp, grid_inp, meas_inp, fore_inp, output_DF):
    ...
    for n in grid_inp["transformers"]:
        ...
        Line_Smax[af.find_n(n["bus_j"], grid_inp["buses"]),
                  af.find_n(n["bus_k"], grid_inp["buses"])] = n["Cap"]
        Line_Vbase[af.find_n(n["bus_j"], grid_inp["buses"]),
                   af.find_n(n["bus_k"], grid_inp["buses"])] = grid_inp["buses"][af.find_n(n["bus_k"], grid_inp["buses"])]["U_kV"]
        Line_Zre[af.find_n(n["bus_j"], grid_inp["buses"]),
                  af.find_n(n["bus_k"], grid_inp["buses"])] = n["R_cc_pu"] * grid_inp["Zbase"]
        Line_Zim[af.find_n(n["bus_j"], grid_inp["buses"]),
                 af.find_n(n["bus_k"], grid_inp["buses"])] = n["X_cc_pu"] * grid_inp["Zbase"]
    ...
    for n in grid_inp["load_elements"]:
        ...
        for t in Time_Set:
            for om in Scen_Omega_Set:
                nn = af.find_n(n["bus"], meas_inp["meas_location"])
                DM_P[n["index"]][t][om] = fore_inp["Dem_P"][nn][t]
                DM_Q[n["index"]][t][om] = fore_inp["Dem_Q"][nn][t]
                if om > 0:
                    DM_P[n["index"]][t][om] = fore_inp["Dem_P"][nn][t] + fore_inp["Dem_P_zeta+"][nn][t]\
                                               - (fore_inp["Dem_P_zeta+"][nn][t] + fore_inp["Dem_P_zeta-"][nn][t])\
                                               * np.random.randint(0, 2)
                    DM_Q[n["index"]][t][om] = fore_inp["Dem_Q"][nn][t] + fore_inp["Dem_Q_zeta+"][nn][t]\
                                               - (fore_inp["Dem_Q_zeta+"][nn][t] + fore_inp["Dem_Q_zeta-"][nn][t])\
                                               * np.random.randint(0, 2)
    print("- Demand data is generated for optimization.")
    ### PV Systems Parameters
    PV_Set = range(np.size(grid_inp["PV_elements"], 0))
    ...
    for n in grid_inp["PV_elements"]:
        ...
        for t in Time_Set:
            for om in Scen_Omega_Set:
                PV_Forecast[n["index"]][t][om] = fore_inp["P_PV"][t] * n["cap_kVA_perPhase"]
                if om > 0:
                    PV_Forecast[n["index"]][t][om] = (fore_inp["P_PV"][t] + fore_inp["P_PV_zeta+"][t]
                                               - (fore_inp["P_PV_zeta+"][t] + fore_inp["P_PV_zeta-"][t])
                                               * np.random.randint(0, 2)) * n["cap_kVA_perPhase"]
    print("- PV data is generated for optimization.")
    ### Storages Parameters
    ST_Set = range(np.size(grid_inp["storage_elements"], 0))
    ST_Inc_Mat = np.zeros((np.size(grid_inp["storage_elements"], 0), grid_inp["Nbus"]))
    ...
    for n in grid_inp["storage_elements"]:
        ...
        ST_Eff_LC[n["index"]] = n["Eff_LC"]
        for om in Scen_Omega_Set:
            ST_SOC_0[n["index"]][om] = n["SOC_max_kWh"] * fore_inp["ST_SOC_0"]
            if om > 0:
                ST_SOC_0[n["index"]][om] = ST_SOC_0[n["index"]][om] * (1 + fore_inp["ST_SOC_zeta+"]\
                               - (fore_inp["ST_SOC_zeta+"] + fore_inp["ST_SOC_zeta-"]) * np.random.randint(0, 2))
    print("- Storage data is generated for optimization.")
    ### Transmission System and Cost Function Parameters
    ConPoint_Set = range(np.size(grid_inp["grid_formers"], 0))
    ConPoint_Inc_Mat = np.zeros((np.size(grid_inp["grid_formers"], 0), grid_inp["Nbus"]))
    ...
    for n in grid_inp["grid_formers"]:
        ...
        for t in Time_Set:
            for om in Scen_Omega_Set:
                ConPoint_Vmag[n["index"]][t][om] = 1.0
                if om > 0:
                    ConPoint_fac_P_pos[n["index"]][t][om] = np.random.randint(0, 2)
                    ConPoint_fac_P_neg[n["index"]][t][om] = 1 - ConPoint_fac_P_pos[n["index"]][t][om]
                    ConPoint_fac_Q_pos[n["index"]][t][om] = np.random.randint(0, 2)
                    ConPoint_fac_Q_neg[n["index"]][t][om] = 1 - ConPoint_fac_Q_pos[n["index"]][t][om]
                    ConPoint_Vmag[n["index"]][t][om] = 1 + fore_inp["Vmag_zeta+"]\
                                        - (fore_inp["Vmag_zeta-"] + fore_inp["Vmag_zeta+"]) * np.random.randint(0, 2)
    print("- Connection point data is generated for optimization.")
    ### Defining Variables
    PROB_DA = Model("PROB_DA")
    ...
    for t, om in itertools.product(Time_Set, Scen_Omega_Set):
        for n in Node_Set:
            # (1a)
            PROB_DA.addConstr(Net_P[n, t, om] == 
                              sum(PV_P[i, t, om] * PV_Inc_Mat[i, n] for i in PV_Set) 
                              + sum(ST_P[s, t, om] * ST_Inc_Mat[s, n] for s in ST_Set)
                              - sum(DM_P[d, t, om] * DM_Inc_Mat[d, n] for d in DM_Set) 
                              + sum(ConPoint_P[f, t, om] * ConPoint_Inc_Mat[f, n] for f in ConPoint_Set))
            # (1b)
            PROB_DA.addConstr(Net_Q[n, t, om] == 
                              sum(PV_Q[i, t, om] * PV_Inc_Mat[i, n] for i in PV_Set) 
                              + sum(ST_Q[s, t, om] * ST_Inc_Mat[s, n] for s in ST_Set) 
                              - sum(DM_Q[d, t, om] * DM_Inc_Mat[d, n] for d in DM_Set) 
                              + sum(ConPoint_Q[f, t, om] * ConPoint_Inc_Mat[f, n] for f in ConPoint_Set))
            # (12c) abd (12d) of Mostafa
            PROB_DA.addConstr(Vmag_sq_over[n, t, om] <= V_max[n] * V_max[n])
            PROB_DA.addConstr(Vmag_sq[n, t, om] >= V_min[n] * V_min[n])
        for n1, n2 in Line_Set:
            # (8a) of Mostafa
            ...
            PROB_DA.addConstr(Line_Q_t[n1, n2, t, om] == - Net_Q[n2, t, om] 
                              ...
                              - 1000 * (Vmag_sq[n1, t, om] + Vmag_sq[n2, t, om]) * Line_b[n1, n2] * (Line_Vbase[n1, n2] ** 2))
           # (8b) of Mostafa
            ...
            PROB_DA.addConstr(Line_Q_b[n1, n2, t, om] == - Net_Q[n2, t, om] 
                              + sum(Line_Q_t[n3, n4, t, om] * np.where(n3 == n2, 1, 0) for n3, n4 in Line_Set))
            # (10) of Mostafa
            PROB_DA.addConstr(Line_f[n1, n2, t, om] * Vmag_sq[n1, t, om] * (Line_Vbase[n1, n2] ** 2) >= Line_P_t[n1, n2, t, om] 
                              * Line_P_t[n1, n2, t, om] 
            ...
            PROB_DA.addConstr(Line_Q_t_hat[n1, n2, t, om] == - Net_Q[n2, t, om] 
                              ...
                              - 1000 * (Vmag_sq_over[n1, t, om] + Vmag_sq_over[n2, t, om]) * Line_b[n1, n2] * (Line_Vbase[n1, n2] ** 2))
            # (11b) of Mostafa
            PROB_DA.addConstr((Vmag_sq_over[n2, t, om] - Vmag_sq_over[n1, t, om]) * (Line_Vbase[n1, n2] ** 2) == 
                              ...
                              + 2 * Line_Zim[n1, n2] * Vmag_sq_over[n1, t, om] * Line_b[n1, n2] * (Line_Vbase[n1, n2] ** 2))
            # (11c) of Mostafa
        ...
        for s in ST_Set:
            ...
            # (22e)
            PROB_DA.addConstr(ST_Q[s, t, om] * ST_Q[s, t, om] + ST_P[s, t, om] * ST_P[s, t, om] <= ST_S_max[s] * ST_S_max[s])
            # (22b)
        ...
        for i in PV_Set:
            ...
            PROB_DA.addConstr(PV_P[i, t, om] * PV_P[i, t, om] 
                              ...
                              * (PV_cap[i] * PV_V_grid[i] * PV_V_conv[i] / PV_X[i]))
            # (27)
            PROB_DA.addConstr(PV_P[i, t, om] * PV_P[i, t, om] + PV_Q[i, t, om] * PV_Q[i, t, om] <= PV_cap[i] * PV_cap[i])
            # PROB_DA.addConstr(PV_Q[i, t, om] == 0)
        for f in ConPoint_Set:
            PROB_DA.addConstr(ConPoint_P[f, t, om] == sum(Line_P_t[n1, n2, t, om] * ConPoint_Inc_Mat[f, n1] for n1, n2 in Line_Set))
            PROB_DA.addConstr(ConPoint_Q[f, t, om] == sum(Line_Q_t[n1, n2, t, om] * ConPoint_Inc_Mat[f, n1] for n1, n2 in Line_Set))
            # first line of (34a)
            PROB_DA.addConstr(OBJ_DA_MARKET[f, t] == - LAMBDA_P_DA_EN * ConPoint_P_DA_EN[f, t] 
                              - LAMBDA_Q_DA_EN * ConPoint_Q_DA_EN[f, t] 
                              + LAMBDA_P_DA_RS_pos * ConPoint_P_DA_RS_pos[f, t]
            ...
            PROB_DA.addConstr(ConPoint_Q[f, t, om] == ConPoint_Q_DA_EN[f, t] 
                              ...
                              - ConPoint_Q_dev_pos[f, t, om] + ConPoint_Q_dev_neg[f, t, om])
            # slack bus definition
            PROB_DA.addConstr(sum(Vmag_sq[n, t, om] * ConPoint_Inc_Mat[f, n] for n in Node_Set) 
            ...
            PROB_DA.addConstr(ConPoint_Q_DA_RS_neg[f, t] <= Line_Smax[0, 1]/2)
        for n1, n2 in Line_Set:
            PROB_DA.addConstr(OBJ_Loss[n1, n2, t, om] >= sum(Line_Zre[n1, n2] * Line_f[n1, n2, t, om] for n1, n2 in Line_Set))
    ### Defining Objective and Solving Problem
        ...
        for k in range(3):
            for n in Node_Set:
                if n > 0:
                    nn = grid_inp["buses"][n]["bus"]
                    nn = af.find_n(nn, meas_inp["meas_location"])
                    meas_inp["P"][nn][k][t] = sum(Solution_PV_P[i, t, 0] * PV_Inc_Mat[i, n] for i in PV_Set) \
                                                + sum(Solution_ST_P[s, t, 0] * ST_Inc_Mat[s, n] for s in ST_Set) \
                                                - sum(DM_P[d, t, 0] * DM_Inc_Mat[d, n] for d in DM_Set)
                    meas_inp["Q"][nn][k][t] = sum(Solution_PV_Q[i, t, 0] * PV_Inc_Mat[i, n] for i in PV_Set) \
                                                + sum(Solution_ST_Q[s, t, 0] * ST_Inc_Mat[s, n] for s in ST_Set) \
                                                - sum(DM_Q[d, t, 0] * DM_Inc_Mat[d, n]  for d in DM_Set)
    DA_P = PROB_DA.getAttr('x', ConPoint_P_DA_EN)
    DA_Q = PROB_DA.getAttr('x', ConPoint_Q_DA_EN)
    DA_RP_pos = PROB_DA.getAttr('x', ConPoint_P_DA_RS_pos)
    DA_RP_neg = PROB_DA.getAttr('x', ConPoint_P_DA_RS_neg)
...
def RT_Optimization(rt_meas_inp, meas_inp, grid_inp, DA_result):
    ...
    for n in grid_inp["load_elements"]:
        DM_Inc_Mat[n["index"], af.find_n(n["bus"], grid_inp["buses"])] = 1
        nn = af.find_n(n["bus"], meas_inp["meas_location"])
        DM_P[n["index"]] = rt_meas_inp["DM_P"][nn]
        DM_Q[n["index"]] = rt_meas_inp["DM_Q"][nn]
    print("- Demand data is generated for optimization.")
    ### PV Systems Parameters
    PV_Set = range(np.size(grid_inp["PV_elements"], 0))
    PV_Inc_Mat = np.zeros((np.size(grid_inp["PV_elements"], 0), grid_inp["Nbus"]))
    ...
    for n in grid_inp["PV_elements"]:
        ...
        PV_V_conv[n["index"]] = n["V_conv_pu"]
        PV_X[n["index"]] = n["X_PV_pu"]
        PV_Forecast[n["index"]] = rt_meas_inp["P_PV"] * n["cap_kVA_perPhase"]
    print("- PV data is generated for optimization.")
    ### Storages Parameters
    ST_Set = range(np.size(grid_inp["storage_elements"], 0))
    ST_Inc_Mat = np.zeros((np.size(grid_inp["storage_elements"], 0), grid_inp["Nbus"]))
    ...
    for s, n in enumerate(grid_inp["storage_elements"]):
        ST_Inc_Mat[n["index"], af.find_n(n["bus"], grid_inp["buses"])] = 1
        ST_S_max[n["index"]] = n["S_max_kVA"]
        ST_SOC_max[n["index"]] = n["SOC_max_kWh"]
        ST_SOC_min[n["index"]] = n["SOC_min_kWh"]
        ...
        ST_Eff_neg[n["index"]] = n["Eff_D"]
        ST_Eff_LC[n["index"]] = n["Eff_LC"]
        ST_SOC_t_1[n["index"]] = rt_meas_inp["ST_SOC_t_1"][s]
        ST_SOC_des[n["index"]] = rt_meas_inp["ST_SOC_des"][s]
    print("- Storage data is generated for optimization.")
    ### Transmission System and Cost Function Parameters
    ConPoint_Set = range(np.size(grid_inp["grid_formers"], 0))
    ConPoint_Inc_Mat = np.zeros((np.size(grid_inp["grid_formers"], 0), grid_inp["Nbus"]))
    ...
    for n in grid_inp["grid_formers"]:
        ConPoint_Inc_Mat[n["index"], af.find_n(n["bus"], grid_inp["buses"])] = 1
        ConPoint_Vmag[n["index"]] = rt_meas_inp["Vmag"][n["index"]]
        ConPoint_fac_P_pos[n["index"]] = rt_meas_inp["fac_P_pos"][n["index"]]
        ConPoint_fac_P_neg[n["index"]] = rt_meas_inp["fac_P_neg"][n["index"]]
        ConPoint_fac_Q_pos[n["index"]] = rt_meas_inp["fac_Q_pos"][n["index"]]
        ConPoint_fac_Q_neg[n["index"]] = rt_meas_inp["fac_Q_neg"][n["index"]]
        ConPoint_P_DA_EN[n["index"]] = rt_meas_inp["ConPoint_P_DA_EN"][n["index"]]
        ConPoint_Q_DA_EN[n["index"]] = rt_meas_inp["ConPoint_Q_DA_EN"][n["index"]]
        ConPoint_P_DA_RS_pos[n["index"]] = rt_meas_inp["ConPoint_P_DA_RS_pos"][n["index"]]
        ConPoint_P_DA_RS_neg[n["index"]] = rt_meas_inp["ConPoint_P_DA_RS_neg"][n["index"]]
        ConPoint_Q_DA_RS_pos[n["index"]] = rt_meas_inp["ConPoint_Q_DA_RS_pos"][n["index"]]
        ConPoint_Q_DA_RS_neg[n["index"]] = rt_meas_inp["ConPoint_Q_DA_RS_neg"][n["index"]]
    print("- Connection point data is generated for optimization.")
    ### Defining Variables
    ...
    for n in Node_Set:
        # (1a)
        PROB_RT.addConstr(Net_P[n] == 
                          sum(PV_P[i] * PV_Inc_Mat[i, n] for i in PV_Set) 
                          + sum(ST_P[s] * ST_Inc_Mat[s, n] for s in ST_Set)
                          - sum(DM_P[d] * DM_Inc_Mat[d, n] for d in DM_Set) 
                          + sum(ConPoint_P[f] * ConPoint_Inc_Mat[f, n] for f in ConPoint_Set))
        # (1b)
        PROB_RT.addConstr(Net_Q[n] == 
                          sum(PV_Q[i] * PV_Inc_Mat[i, n] for i in PV_Set) 
                          + sum(ST_Q[s] * ST_Inc_Mat[s, n] for s in ST_Set) 
                          - sum(DM_Q[d] * DM_Inc_Mat[d, n] for d in DM_Set) 
                          + sum(ConPoint_Q[f] * ConPoint_Inc_Mat[f, n] for f in ConPoint_Set))
        # (12c) abd (12d) of Mostafa
        PROB_RT.addConstr(Vmag_sq_over[n] <= V_max[n] * V_max[n])
        PROB_RT.addConstr(Vmag_sq[n] >= V_min[n] * V_min[n])
    for n1, n2 in Line_Set:
        # (8a) of Mostafa
        ...
        PROB_RT.addConstr(Line_Q_b_sq_max[n1, n2] >= 
                          - (Line_Q_b_hat[n1, n2] - Vmag_sq_over[n2] * Line_b[n1, n2]
                            * (Line_Vbase[n1, n2] ** 2) * 1000))       
        PROB_RT.addConstr(Line_Q_b_sq_max[n1, n2] >= 
                            (Line_Q_b_over[n1, n2] - Vmag_sq[n2] * Line_b[n1, n2] 
                            * (Line_Vbase[n1, n2] ** 2) * 1000))
        PROB_RT.addConstr(Line_Q_b_sq_max[n1, n2] >= 
                            - (Line_Q_b_over[n1, n2] - Vmag_sq[n2] * Line_b[n1, n2] 
                            * (Line_Vbase[n1, n2] ** 2) * 1000))        
        ...
        PROB_RT.addConstr(Line_P_t_sq_max[n1, n2] >= - Line_P_t_over[n1, n2])        
        PROB_RT.addConstr(Line_Q_t_sq_max[n1, n2] >= 
                          (Line_Q_t_hat[n1, n2] + Vmag_sq_over[n1] * Line_b[n1, n2] 
                            * (Line_Vbase[n1, n2] ** 2) * 1000))
        PROB_RT.addConstr(Line_Q_t_sq_max[n1, n2] >= 
                          - (Line_Q_t_hat[n1, n2] + Vmag_sq_over[n1] * Line_b[n1, n2] 
                            * (Line_Vbase[n1, n2] ** 2) * 1000))
        PROB_RT.addConstr(Line_Q_t_sq_max[n1, n2] >= 
                          (Line_Q_t_over[n1, n2] + Vmag_sq[n1] * Line_b[n1, n2] 
                            * (Line_Vbase[n1, n2] ** 2) * 1000)) 
        PROB_RT.addConstr(Line_Q_t_sq_max[n1, n2] >= 
                          - (Line_Q_t_over[n1, n2] + Vmag_sq[n1] * Line_b[n1, n2] 
                            * (Line_Vbase[n1, n2] ** 2) * 1000))         
    ...
    try:
        ...
        dres = (Solution_P_dev_pos[0] + Solution_P_dev_neg[0] + Solution_Q_dev_pos[0] + Solution_Q_dev_neg[0] > rt_meas_inp["delta"])
    except:
